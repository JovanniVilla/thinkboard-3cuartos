...
const ProjectDetailPage = () => {
  ...
  const handlePostComment = async () => {
    ...
    try {
      const res = await api.put(`/projects/${id}`, { activities: nextActivities });
      setProject(res.data);
      setCommentText("");
    } catch (error) {
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-4 py-3 max-w-7xl mx-auto">
          <div className="flex items-center gap-3 self-start sm:self-auto">
            ...
            <div className="flex items-center gap-2 text-sm font-semibold text-base-content/70 bg-base-200 px-3 py-1.5 rounded-full">
              <FolderKeyIcon className="size-4" style={{ color: project.color }} />
              <span className="truncate max-w-[150px]">Proyecto</span>
            </div>
            {saving && <span className="loading loading-spinner loading-xs text-primary ml-2"></span>}
          </div>
          <div className="flex items-center gap-2 self-end sm:self-auto">
            <ThemeToggle />
            <button className="btn btn-ghost btn-sm text-error" onClick={handleDeleteProject} title="Eliminar proyecto">
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-8">
          ...
          <div className="flex flex-wrap items-center gap-4 p-4 bg-base-100 rounded-2xl border border-base-content/10 shadow-sm">
            ...
            <div className="dropdown dropdown-hover">
              <div tabIndex={0} role="button" className="btn btn-sm btn-ghost gap-2 border border-base-content/10 hover:border-base-content/30 rounded-xl">
                <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: projectStatuses.find(s => s.name === project.status)?.color || "#6B7280" }}></span>
                ...
                {projectTypes.map((pt) => (
                  <li key={pt._id}>
                    <button onClick={() => {
                      ...
                      <span className="w-3 h-3 rounded-full" style={{ backgroundColor: pt.color }}></span>
                      {pt.name}
                    </button>
                  </li>
                ))}
            ...
            <div className="dropdown dropdown-hover">
              <div tabIndex={0} role="button" className="btn btn-sm btn-ghost gap-2 border border-base-content/10 hover:border-base-content/30 rounded-xl">
                ...
                <span>{project.assignedTo === "Sin asignar" ? "Responsable" : project.assignedTo}</span>
              </div>
              <ul tabIndex={0} className="dropdown-content z-[60] menu p-2 shadow-xl bg-base-100 rounded-box w-56 border border-base-content/10 max-h-64 overflow-y-auto mt-1">
                <li><button onClick={() => { setProject({ ...project, assignedTo: "Sin asignar" }); handleSaveProject({ assignedTo: "Sin asignar" }); document.activeElement?.blur(); }}>Sin asignar</button></li>
                {accountsList.map((accName) => (
                  <li key={accName}><button onClick={() => { setProject({ ...project, assignedTo: accName }); handleSaveProject({ assignedTo: accName }); document.activeElement?.blur(); }}>{accName}</button></li>
                ))}
              </ul>
            </div>
            ...
            {isEditingDescription ? (
              <div className="animate-in fade-in zoom-in-95 duration-200">
                <MarkdownEditor
                  ...
                  onSave={() => { setIsEditingDescription(false); handleSaveProject(); }}
                ...
                {tasks.map(task => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-semibold truncate ${task.status === "Completado" ? "line-through opacity-60" : ""}`}>
                        {task.title}
                      </p>
                    </div>
        ...
        <div className="w-full lg:w-96 flex-shrink-0 flex flex-col gap-6">
          ...
          <div className="bg-base-100 rounded-2xl border border-base-content/10 shadow-sm overflow-hidden">
            ...
            <div className="p-4 space-y-4">
              <div>
                <label className="text-xs font-semibold text-base-content/60 mb-1 block">Brief del Proyecto</label>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) => setProject({ ...project, briefUrl: e.target.value })}
                    onBlur={() => handleSaveProject()}
                  />
                  {project.briefUrl && (
                    <a href={project.briefUrl} target="_blank" rel="noopener noreferrer" className="btn btn-sm btn-square btn-ghost text-primary">
              ...
              <div>
                <label className="text-xs font-semibold text-base-content/60 mb-1 block">Carpeta en Drive / Archivos</label>
                <div className="flex items-center gap-2">
                  <input
                    type="url"
                    className="input input-sm input-bordered w-full"
                    ...
                    onChange={(e) => setProject({ ...project, folderUrl: e.target.value })}
                    onBlur={() => handleSaveProject()}
                  />
                  {project.folderUrl && (
                    <a href={project.folderUrl} target="_blank" rel="noopener noreferrer" className="btn btn-sm btn-square btn-ghost text-primary">
          ...
          <div className="bg-base-100 rounded-2xl border border-base-content/10 shadow-sm overflow-hidden">
            ...
            <div className="p-4">
              <label className="text-xs font-semibold text-base-content/60 mb-1 block">Asignar nuevas tareas a:</label>
              <select 
              ...
              <p className="text-[10px] text-base-content/50 mt-2 leading-tight">
                Al crear una tarea con este proyecto, se le asignará automáticamente a este usuario si no se elige otro explícitamente.
              </p>
            </div>
          </div>
